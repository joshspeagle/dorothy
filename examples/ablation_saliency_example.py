#!/usr/bin/env python
"""
Example: Ablation-Based Saliency Analysis for Stellar Parameter Predictions.

This script demonstrates how to compute and visualize ablation-based saliency
maps showing which wavelengths are most important for predicting each stellar
parameter using a masking/occlusion approach.

The ablation approach masks contiguous spectral blocks using a sliding window
and measures the impact on predictions. This provides a complementary view to
...
Usage:
    # Using real data from variant5 checkpoint (default: training distribution weighting)
    python examples/ablation_saliency_example.py \
    ...
    python examples/ablation_saliency_example.py --demo

    # With single fixed block size instead of training distribution
    python examples/ablation_saliency_example.py --demo --single-block-size --block-size 50
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute and visualize ablation-based saliency maps"
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/ablation_saliency"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Block size in pixels for single-block ablation (default: 5)",
    )
    parser.add_argument(
        "--single-block-size",
        action="store_true",
        help="Use single block size instead of training distribution (faster but less accurate)",
    )
    parser.add_argument(
        "--n-block-sizes",
        type=int,
        default=21,
        help="Number of block sizes for training distribution mode (default: 21)",
    )
    parser.add_argument(
        "--f-max",
        type=float,
        default=0.5,
        help="Maximum block fraction for training distribution mode (default: 0.5)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=64,
        help="Batch size for GPU computation (default: 64)",
    )
    ...
    analyzer = AblationSaliencyAnalyzer(
        model, device=device, batch_size=args.batch_size
    )
    print(f"Ablation saliency analyzer created on device: {analyzer.device}")
    ...
    print(
        f"Spectrum shape: {X.shape}, wavelength range: {wavelength.min():.0f}-{wavelength.max():.0f} A"
    )

    # Compute saliency
    if args.single_block_size:
        print(f"Computing ablation saliency with block_size={args.block_size}...")
        ...
        print(
            f"Computing ablation saliency with training distribution weighting "
            f"(n_block_sizes={args.n_block_sizes}, f_max={args.f_max})..."
        )
        result = analyzer.compute_saliency(
            X,
            wavelength,
            ...
            n_block_sizes=args.n_block_sizes,
            f_max=args.f_max,
            gaia_id=gaia_id,
        )
        print(f"  Block sizes used: {result.block_size}")
    ...
    # Create filename suffix based on block size mode
    block_suffix = f"b{args.block_size}" if args.single_block_size else "dist"

    for param in key_params:
        output_path = args.output_dir / f"ablation_saliency_{param}_{block_suffix}.png"
        print(f"Plotting {param} ablation saliency -> {output_path}")
    ...
    # Plot heatmap
    heatmap_path = args.output_dir / f"ablation_saliency_heatmap_{block_suffix}.png"
    print(f"Plotting heatmap -> {heatmap_path}")
    fig = plot_ablation_saliency_heatmap(result, output_path=heatmap_path)
    plt.close(fig)

    print(f"\nAblation saliency analysis complete! Outputs saved to: {args.output_dir}")
